...
class Plugin {
	constructor($container, handleColorChange, options) {
...
function tinycolorpicker($container, handleChangeColor, options) {
    return new Plugin($container, handleChangeColor, options);
}

class TinyColorpicker extends Component {
	componentDidMount() {
		const $picker = React.findDOMNode(this);
		const picker  = tinycolorpicker($picker, this.props.handleColorChange)
	}

	render() {
		return (
			<div className="tinycolorpicker">
